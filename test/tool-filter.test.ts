...
describe('selecting tools', () => {
  it('narrows tools/list to an allow list', async () => {
    expect(await toolNames({ allowTools: 'search_links,get_link' })).toEqual([
      'get_link',
      'search_links',
    ]);
  });
  ...
  it('removes a whole family with a prefix pattern', async () => {
    const names = await toolNames({ denyTools: 'bulk_*' });
    expect(names.some((n) => n.startsWith('bulk_'))).toBe(false);
    expect(names).toHaveLength(
      ALL_TOOLS.length - ALL_TOOLS.filter((t) => t.startsWith('bulk_')).length
    );
  });
  ...
  it('subtracts the deny list from the allow list', async () => {
    expect(
      await toolNames({
        allowTools: 'list_*',
        denyTools: 'list_rss_subscriptions',
      })
    ).toEqual(['list_collections', 'list_tags']);
  });
  ...
  it('lets the preset compose with extra names', async () => {
    expect(await toolNames({ allowTools: 'essential,get_dashboard' })).toEqual(
      [...ESSENTIAL_TOOLS, 'get_dashboard'].toSorted()
    );
  });
...
describe('refusing an unusable list', () => {
  ...
  it('applies the same rule to the deny list', () => {
    expect(() => build({ denyTools: 'delet_link' })).toThrow(
      /LINKWARDEN_DENY_TOOLS: no tool matches "delet_link"/
    );
  });
...
describe('together with read-only mode', () => {
  ...
  it('names read-only as the reason, rather than calling the tool unknown', () => {
    ...
    let thrown: unknown;
    try {
      build({ ...readOnly, allowTools: 'delete_link' });
    } catch (error) {
      thrown = error;
    }
    const message = (thrown as Error).message;
    expect(message).toContain('LINKWARDEN_READ_ONLY');
  ...
  it('does not apply the write-tool rule to the deny list', async () => {
    ...
    expect(await toolNames({ ...readOnly, denyTools: 'delete_link' })).toEqual(
      READ_TOOLS.toSorted()
    );
  });
});
