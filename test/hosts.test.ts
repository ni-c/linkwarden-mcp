 ...
 *
 * Linkwarden fetches every URL this server hands it — `create_link` and
 * `update_link` through the headless-browser preserver, `create_rss_subscription`
 * immediately — and `get_link_content` reads the preserved page back out. That
 * makes an unchecked URL a request from inside Linkwarden's network whose
...
describe('internalHostKind', () => {
  ...
  it.each([
    // A self-hosted bookmark manager is pointed at the LAN on purpose: the
  ...
  // leaving it on desynchronised the dotted-quad fold from what isIP agreed to.
  it.each([
...
describe('assertFetchableUrl', () => {
  ...
  it('refuses a name that resolves to loopback', async () => {
    ...
    await expect(
      assertFetchableUrl('http://bookmark.attacker.example/x')
  ...
  it('allows a name it cannot resolve, which Linkwarden may still reach', async () => {
